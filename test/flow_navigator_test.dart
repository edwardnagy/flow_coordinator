...
class TestFlowNavigator implements FlowNavigator {
  ...
  @override
  Future<bool> maybePopInternally<T extends Object?>([T? result]) async => false;
}
...
void main() {
  group('FlowNavigator', () {
    ...
    testWidgets('maybeOf with listen true returns null when not found', (tester) async {
      await tester.pumpWidget(Container());
      final context = tester.element(find.byType(Container));
    ...
    testWidgets('Finds FlowNavigator when provided with listen', (tester) async {
      final navigator = TestFlowNavigator();
      await tester.pumpWidget(
        FlowNavigatorScope(
            flowNavigator: navigator,
    ...
    test('FlowNavigatorScope updateShouldNotify returns true when navigator changes', () {
      final nav1 = TestFlowNavigator();
      final nav2 = TestFlowNavigator();
      final scope1 = FlowNavigatorScope(flowNavigator: nav1, child: Container());
      final scope2 = FlowNavigatorScope(flowNavigator: nav2, child: Container());
      expect(scope2.updateShouldNotify(scope1), true);
    });

    test('FlowNavigatorScope updateShouldNotify returns false when navigator same', () {
