...
mixin FlowCoordinatorMixin<T extends StatefulWidget> on State<T> {
  ...
  void didChangeDependencies() {
    ...
    if (parentRouteInformationProvider != _parentRouteInformationProvider) {
      ...
      final receivedRouteInformation = parentRouteInformationProvider
          .childValueListenable.value
          ?.consumeOrNull();
      if (receivedRouteInformation != null &&
          (_routeInformationProvider.consumedValueListenable.value != null ||
              this.initialRouteInformation == null ||
              receivedRouteInformation.isNotDefault())) {
        setNewRouteInformation(receivedRouteInformation);
      }
...
extension on RouteInformation {
  bool isNotDefault() =>
      uri != Uri.parse(Navigator.defaultRouteName) || state != null;
}
